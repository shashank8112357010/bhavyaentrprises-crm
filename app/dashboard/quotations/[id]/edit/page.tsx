...
// Service imports
import { getQuotationById, updateQuotation, UpdateQuotationParams } from "@/lib/services/quotations";
import { getAllRateCards } from "@/lib/services/rate-card";
import { getTicketsForSelection, TicketForSelection } from "@/lib/services/ticket";
...
const RateCardItemsTable: React.FC<RateCardItemsTableProps> = ({ items, onItemChange, onRemoveItem }) => {
  return (
        ...
        <TableBody>
          {items.map((item, index) => (
            <TableRow key={item.rateCardId}>
              ...
              <TableCell>
                <Button variant="ghost" size="icon" onClick={() => onRemoveItem(index)} aria-label="Remove item">
...
const CreateClientDialog: React.FC<CreateClientDialogProps> = ({ isOpen, onOpenChange, onClientCreated }) => {
  ...
  const [dialogFormData, setDialogFormData] = useState<Partial<CreateClientFormData>>({
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setDialogFormData(prev => ({ ...prev, [name]: type === 'number' ? parseInt(value, 10) : value }));
  ...
  const handleDialogSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onOpenChange(false);
      setDialogFormData({ type: "Bank", contractStatus: "Active", lastServiceDate: new Date().toISOString().split('T')[0] });
      setErrors({});
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={handleDialogSubmit} className="space-y-4 py-4">
          <div>
            ...
            <Input id="name_edit_dialog" name="name" value={dialogFormData.name || ""} onChange={handleInputChange} />
            {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
          </div>
          <div>
            <Label htmlFor="type_edit_dialog">Client Type</Label>
            <Select name="type" value={dialogFormData.type || "Bank"} onValueChange={(value) => handleSelectChange("type", value)}>
              <SelectTrigger id="type_edit_dialog"><SelectValue placeholder="Select client type" /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Input id="totalBranches_edit_dialog" name="totalBranches" type="number" value={dialogFormData.totalBranches || ""} onChange={handleInputChange} />
          ...
          <div>
            ...
            <Input id="contactPerson_edit_dialog" name="contactPerson" value={dialogFormData.contactPerson || ""} onChange={handleInputChange} />
            {errors.contactPerson && <p className="text-red-500 text-xs mt-1">{errors.contactPerson}</p>}
          </div>
          <div>
            <Label htmlFor="contactPhone_edit_dialog">Contact Phone</Label>
            <Input id="contactPhone_edit_dialog" name="contactPhone" value={dialogFormData.contactPhone || ""} onChange={handleInputChange} />
            {errors.contactPhone && <p className="text-red-500 text-xs mt-1">{errors.contactPhone}</p>}
          </div>
          <div>
            <Label htmlFor="contactEmail_edit_dialog">Contact Email (Optional)</Label>
            <Input id="contactEmail_edit_dialog" name="contactEmail" type="email" value={dialogFormData.contactEmail || ""} onChange={handleInputChange} />
            {errors.contactEmail && <p className="text-red-500 text-xs mt-1">{errors.contactEmail}</p>}
          </div>
          <div>
            <Label htmlFor="contractStatus_edit_dialog">Contract Status</Label>
            <Select name="contractStatus" value={dialogFormData.contractStatus || "Active"} onValueChange={(value) => handleSelectChange("contractStatus", value)}>
              <SelectTrigger id="contractStatus_edit_dialog"><SelectValue placeholder="Select status" /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            <Label htmlFor="lastServiceDate_edit_dialog">Last Service Date</Label>
            <Input id="lastServiceDate_edit_dialog" name="lastServiceDate" type="date" value={dialogFormData.lastServiceDate || ""} onChange={handleInputChange} />
          ...
          <div>
            ...
            <Input id="gstn_edit_dialog" name="gstn" value={dialogFormData.gstn || ""} onChange={handleInputChange} />
          </div>
          <div>
            <Label htmlFor="avatar_edit_dialog">Avatar URL (Optional)</Label>
            <Input id="avatar_edit_dialog" name="avatar" value={dialogFormData.avatar || ""} onChange={handleInputChange} />
          </div>
          <div>
            <Label htmlFor="initials_edit_dialog">Initials (Optional)</Label>
            <Input id="initials_edit_dialog" name="initials" value={dialogFormData.initials || ""} onChange={handleInputChange} />
          </div>
          <DialogFooter>
...
const EditQuotationPage: React.FC<EditQuotationPageProps> = ({ params }) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        toast({ title: "Error", description: "Could not load required data (rate cards or tickets).", variant: "destructive" });
      } finally {
  ...
  useEffect(() => {
    if (quotationId) {
      ...
      getQuotationById(quotationId)
        .then((data: any) => {
          const transformedRateCardDetails: QuotationRateCardItem[] = (allRateCards.length > 0 && data.rateCardDetails ? data.rateCardDetails : []).map((item: any) => {
  ...
  const handleRemoveRateCardItem = (indexToRemove: number) => {
    setFormData(prev => {
      if (!prev || !prev.rateCardDetails) return prev;
      const updatedItems = prev.rateCardDetails.filter((_, index) => index !== indexToRemove);
    ...
    toast({ title: "Item Removed", description: "Rate card item removed from quotation." });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: `Quotation ${updatedData.quoteNo} updated.` });
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm("Are you sure you want to delete this quotation?")) {
        try {
            await apiRequest(`/api/quotations/${formData.id}`, { method: "DELETE" });
  ...
  if (isLoading && !formData.id) return <div className="flex justify-center items-center h-screen"><Loader2 className="h-8 w-8 animate-spin" /> Loading...</div>;
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Edit Quotation {formData.quoteNo || `(ID: ${formData.id})`}</h1>
          ...
          <Card>
            ...
            <CardContent className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="quotationNumberDisplay" value={formData.quoteNo || ''} readOnly />
              </div>
              <div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    setFormData(prev => ({ ...prev, ticketId: value === "" ? null : value }));
                  }}
            ...
            <CardContent>
              ...
              <div className="mt-6 space-y-2">
                 <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      if (formData.pdfUrl && typeof formData.pdfUrl === 'string' && formData.pdfUrl.trim() !== '') {
                        window.open(formData.pdfUrl, '_blank');
                      } else {
                        toast({
                          title: "PDF Not Available",
                          description: "The PDF for this quotation has not been generated or the URL is missing. Please save the quotation or try generating the PDF if applicable.",
                          variant: "default",
                        });
                      }
                    }}
