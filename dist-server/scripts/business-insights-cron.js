...
async function generateExcel(tickets) {
    ...
    tickets.filter(t => t.status === 'billing_pending').forEach(ticket => {
        ...
        const expenseTotal = ticket.expenses.reduce((sum, e) => sum + (e.amount || 0), 0);
    ...
    tickets.filter(t => t.status === 'new' || t.status === 'in_progress').forEach(ticket => {
        ...
        const totalExpenseDone = ticket.expenses.reduce((sum, e) => sum + (e.amount || 0), 0);
...
async function main() {
    try {
        ...
        tickets.forEach(ticket => {
            ...
            const expense = ticket.expenses.reduce((sum, e) => sum + (e.amount || 0), 0);
            const diff = expected - expense;
            const breakdownLine = `Ticket: ${ticket.ticketId || ticket.id}\n` +
